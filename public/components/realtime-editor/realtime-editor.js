...
realtimeEditor.prototype.loadText = function () {
	...
	if (document.activeElement.id !== this.editor.id) {
		...
		//console.log("Received Data: ", this.text.author);
...
realtimeEditor.prototype.update = function (data) {
	...
	if (target !== null) {
		...
		if (data.type === 'modifyLine') { // patch existing line
			...
			console.log("WHere is RECEIVING ", data.savedLines[0].author);

			if (data.savedLines[0].author == 1615) {
				if (jQuery('.modal').css('display') == 'none') {
					jQuery('.open-scratch-pad').click();
				}
			}
